...
AnimationMenu.OnServerPostInit = function()
	...
	if cfg.OnServerPostInit then
		...
		recordStore:Save()
		tes3mp.LogAppend(enumerations.log.ERROR, "AnimationMenu Terminate")
	end
end
...
AnimationMenu.OnGUIAction = function(eventStatus, pid, idGui, data)
	...
	if idGui == guiID.animMenu then
		...
		elseif tonumber(data) == 9 then
			...
			logicHandler.RunConsoleCommandOnPlayer(pid, "player->removespell sittingAnim_paralyze", false)
...
-- EVENTS --
------------
customEventHooks.registerHandler("OnServerPostInit", AnimationMenu.OnServerPostInit)
...
customEventHooks.registerValidator("OnPlayerDisconnect", AnimationMenu.OnPlayerDisconnect)

customEventHooks.registerHandler("OnGUIAction", AnimationMenu.OnGUIAction)

customCommandHooks.registerCommand("anim", AnimationMenu.showAnimMenu)
